...
class MeanSquaredRegression(chainer.Chain):
    ...
    def __call__(self, x, t):
        ...
        t_1d = t.data.reshape((t.data.shape[0],))
...
#     print("{}\tFeatures extracted".format(strftime("%y-%m-%d_%H:%M:%S")))
    ...
    features_loc = "c:/Users/Andrew/git/HumourDetection/HumourDetection/src/evocation_estimation/usf_feats_all3.pkl"
#     features_loc = "c:/Users/Andrew/git/HumourDetection/HumourDetection/src/evocation_estimation/eat_feats_all3.pkl"
    ...
    feat_labels = ["w2v sim", "glove sim","w2g energy",
                   "max autoex sim", "avg autoex sim",
                   "max stimuli betweenness", "total stimuli betweenness", "avg stimuli betweenness",
                   "max response betweenness", "total response betweenness", "avg response betweenness",
                   "max stimuli load", "total stimuli load", "avg stimuli load",
                   "max response load", "total response load", "avg response load",
                   "dirrel", "stimuli lexvector", "response lexvector",
                   "wup max", "wup avg", "path max", "path avg", "lch max", "lch avg"]
    offset_labels = ["w2v offset", "glove offset", "w2g offset"]
    output_dir = "c:/Users/Andrew/Desktop/results/usf"
...
#     for offset in offset_vects:
    ...
    for feature in all_labels:
        print("{}\tStarting test for -{}".format(strftime("%y-%m-%d_%H:%M:%S"), feature))
        output_loc = join(output_dir, "minus {} (w2v offsets)".format(feature)) 
        ...
        num_features = len(feature_vects[feature][0])
...
#         train, test = split_dataset_random(data, train_size)   
        ...
        print("{}\tStarting training {}".format(strftime("%y-%m-%d_%H:%M:%S"), feature))
        train_iter = chainer.iterators.SerialIterator(train, args.batchsize)
        test_iter = chainer.iterators.SerialIterator(validation, args.batchsize,
...
#         trainer = training.Trainer(updater, (args.epoch, 'epoch'), out=args.out)
    ...
    #     chainer.serializers.save_npz("eat.model", model)
        print("{}\tTraining {} finished".format(strftime("%y-%m-%d_%H:%M:%S"), feature))
