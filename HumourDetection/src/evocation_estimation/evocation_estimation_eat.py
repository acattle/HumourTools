...
from evocation_reader import EATGraph
...
def main():
    ...
    association_chunks = [[associations[i] for i in xrange(len(associations)) if (i % 5) == r] for r in range(5)]
    p=Pool(5)
    data = []
    target = []
    ...
    trainer.extend(
        extensions.PlotReport(['main/loss', 'validation/main/loss'], 'epoch',
                              file_name='loss_eat.png'))
    trainer.extend(
        extensions.PlotReport(['main/accuracy', 'validation/main/accuracy'],
                              'epoch', file_name='accuracy_eat.png'))
