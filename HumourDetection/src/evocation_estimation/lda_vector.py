...
class GensimDocSummarization(object):
    def __init__(self, word_ids_loc, tfidf_model_loc, dimensions=300, word_separator=" ", cache=True):
    ...
    def get_vector(self,document):
        ...
        if (self._cache != None) and (document in self._cache):
            ...
            tokens = document.split(self.word_separator)
...
class LDAVectorizer(GensimDocSummarization):
    def __init__(self, lda_fileloc, word_ids_loc, tfidf_model_loc, word_separator=" "):
        super(LDAVectorizer, self).__init__(word_ids_loc, tfidf_model_loc, word_separator=word_separator)
        self.model = LdaModel.load(lda_fileloc, mmap="r")

class LSIVectorizer(GensimDocSummarization):
    def __init__(self, lsi_fileloc, word_ids_loc, tfidf_model_loc, word_separator=" "):
        super(LSIVectorizer, self).__init__(word_ids_loc, tfidf_model_loc, word_separator=word_separator)
