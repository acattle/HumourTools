...
#     targets = targets*100 #seems to help
    ...
    print(f"{label} train len: {len(train_X)}")
    from word_associations.association_feature_extractor import AssociationFeatureExtractor
    fe = AssociationFeatureExtractor(lda_model_getter=get_wikipedia_lda,
                                                      ...
                                                      verbose=True,
                                                      low_memory=False)
    train_X = fe.fit_transform(train_X)
    print(f"{label} features shape: {train_X.shape}")
    print(train_X)
    
    np.save(f"{label} train_X",train_X)
    np.save(f"{label} train_y",train_y)
    ...
    print(f"{label} features shape: {test_X.shape}")
    print(test_X)
    
    np.save(f"{label} test_X", test_X)
    np.save(f"{label} test_y", test_y)
...
#                                                      repeat=False, shuffle=False)
    ...
    trainer = training.Trainer(updater, (args.epoch, 'epoch'), out="chainer-out")
...
if __name__ == '__main__':
    ...
    from util.dataset_readers.association_readers.xml_readers import EAT_XML_Reader, USF_XML_Reader, EvocationDataset
    ...
    evoc = [((wp[0].split(".")[0], wp[0].split(".")[1]), stren) for wp, stren in evoc] #remove synset information
